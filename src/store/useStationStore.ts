...
interface StationState {
  ...
  login: (user: UserProfile, stats?: { score?: number; tasksDone?: number; streak?: number; rank?: number }) => void;
  logout: () => void;
  boostRank: (amount: number) => void;
  completeTask: () => void;
  setStats: (stats: { score?: number; tasksDone?: number; streak?: number; rank?: number; weeklyGoalProgress?: number }) => void;
...
export const useStationStore = create<StationState>((set) => ({
  ...
  login: (user, stats) => set((s) => ({
    isLoggedIn: true,
    user,
    domain: user.domain,
    theme: user.domain,
    score: stats?.score !== undefined ? stats.score : s.score,
    tasksDone: stats?.tasksDone !== undefined ? stats.tasksDone : s.tasksDone,
    streak: stats?.streak !== undefined ? stats.streak : (s.streak || 1),
    rank: stats?.rank !== undefined ? stats.rank : s.rank,
  })),
  logout: () => set({ isLoggedIn: false, user: null }),
  boostRank: (amount) => set((s) => ({ rank: Math.max(1, s.rank - amount) })),
  completeTask: () => set((s) => ({
    tasksDone: s.tasksDone + 1,
    weeklyGoalProgress: Math.min(100, s.weeklyGoalProgress + 5),
    score: s.score + 10,
    rank: Math.max(1, s.rank - 5),
  })),
  setStats: (stats) => set((s) => ({
    score: stats.score !== undefined ? stats.score : s.score,
    tasksDone: stats.tasksDone !== undefined ? stats.tasksDone : s.tasksDone,
    streak: stats.streak !== undefined ? stats.streak : s.streak,
    rank: stats.rank !== undefined ? stats.rank : s.rank,
    weeklyGoalProgress: stats.weeklyGoalProgress !== undefined ? stats.weeklyGoalProgress : s.weeklyGoalProgress,
  })),
