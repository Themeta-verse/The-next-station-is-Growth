...
import { TrendingUp, Flame, CheckCircle, Target, Zap, ChevronRight, Square, Check, X, Brain, Trophy, Medal, PieChart, Plus, Trash2, BarChart3, Search, Loader2, BookOpen, ArrowRight, Sparkles, Award } from 'lucide-react';
...
export default function Home() {
  ...
  const toggleTodo = (i: number) => {
    ...
    if (!n[i]) {
      n[i] = true;
      completeTask();
      recordProgress({ scoreDelta: 15, taskCompleted: true });
      checkCloseness();
    } else {
      n[i] = false;
    }
  ...
  const toggleCustomTask = (i: number) => {
    setCustomTasks(prev => prev.map((t, idx) => {
      if (idx === i) {
        if (!t.done) {
          completeTask();
          recordProgress({ scoreDelta: 15, taskCompleted: true });
